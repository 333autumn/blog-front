        ...
        <span v-for="category in categories">
          <el-badge :value="category.articleNums" class="item" type="info">
            <el-tag
              ...
              effect="plain"
              type="info"
              @click="getByCategory(category.categoryId)"
            >
...
export default {
  ...
  methods: {
    ...
    getByCategory(categoryId) {
      getArticleListByCategory(categoryId).then((response) => {
        this.articles = response.data.articles;
      });
    },
    goToBlog(article) {
...
.banner {
  ...
  top: 0;
  left: 0;
  background: url("../assets/imgs/category.jpg") center center / cover no-repeat;
  width: 100%;
  height: 50%;
}
.title {
